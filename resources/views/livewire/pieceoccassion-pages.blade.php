    ...
    <section class="py-10 bg-gray-50 font-poppins dark:bg-gray-800 rounded-lg">
     ...
     <!--catégories-->
               ...
               @foreach ($catalogue as $category)
                <li class="mb-4" wire:key="{{ $category->id }}">
                  <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                    <input type="checkbox" wire:model.live="selected_catalogue" id=" {{ $category->slug }}" value="{{ $category->id }}" class="w-4 h-4 mr-2">
         ...
         <!--marque-->
            ...
            <!--Statut du Produit-->
            <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
              <h2 class="text-2xl font-bold dark:text-gray-400">Statut du Produit</h2>
              <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
              <ul>
                <li class="mb-4">
                  <label for="featured" class="flex items-center dark:text-gray-300">
                    ...
                    <span class="text-lg dark:text-gray-400">Produit recommandé</span>
                  </label>
                </li>
                <li class="mb-4">
                  <label for="on_sale" class="flex items-center dark:text-gray-300">
                    ...
                    <span class="text-lg dark:text-gray-400">En Promotion</span>
                  </label>
                </li>
              </ul>
            </div>
          <!--filtre prix-->
            ...
            <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
              ...
              <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
              <div>
                <div class="font-semibold">{{ Number::currency($price_range,'TND') }}</div>
           ...
           <!--Menu déroulement (Trier par Dernier/Prix) --> 
          </div>
              ...
              <div class="items-center justify-between hidden px-3 py-2 bg-gray-100 md:flex dark:bg-gray-900 ">
                <div class="flex items-center justify-between">
                  <select wire:model.live="sort" class="block w-40 text-base bg-gray-100 cursor-pointer dark:text-gray-400 dark:bg-gray-900">
                    <option value="latest">Trier par dernier</option>
          ...
          <!-- les produits (piéces occassion ) -->
               @foreach ($pieceoccassion as $pieceoccassion)
               <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key="{{ $pieceoccassion->id }}">
                <div class="border border-gray-300 dark:border-gray-700">
                  <div class="relative bg-gray-200">
                    <a href="/pieceoccassion/{{ $pieceoccassion ->slug }}" class="">
                      <img src="{{ url('storage/'.$pieceoccassion->images[0]) }}" 
                  ...
                  <div class="p-3 ">
                    ...
                    <p class="text-lg ">
                      <span class="text-green-600 dark:text-green-600">
                        {{ Number::currency($pieceoccassion->price, 'TND') }}
                      </span>
                    </p>
                  </div>
                  <!--Boutton ajouter au panier pour produits -->
                  <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">
                    <a wire:click.prevent='addToCart({{ $pieceoccassion->id }})' href="#" class="text-gray-500 
                      flex items-center space-x-2 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" 
                      class="w-4 h-4 bi bi-cart3 " viewBox="0 0 16 16">
                        <path d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .49.
                        598l-1 5a.5.5 0 0 1-.465.401l-9.397.472L4.415 11H13a.5.5 0 0 1 0 1H4a.5.5 0 0 1-.
                      ...
                      </svg><span wire:loading.remove wire:target='addToCart({{ $pieceoccassion->id }})'>
                        Ajouter Au Panier</span><span wire:loading wire:target='addToCart({{ $pieceoccassion->id }})'>Ajouté...</span>
                    </a>
                  </div>
                </div>
              </div>
